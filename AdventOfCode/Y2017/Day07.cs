...
namespace Puzzles.AdventOfCode.Y2017
{
    ...
    public class Day07
    {
        ...
        /// they don't do this in an orderly fashion; by the time you're done, you're not sure which program gave which information.
        /// </summary>
        [TestCase("tknk", FirstTowerCase)]
        [TestCase("rqwgj", TowerPuzzleInput)]
        public void Part1(string expectedBottom, string input)
        {
            var towerLevels = CompileTowerLevels(input);

            var foundation = towerLevels.Single(l => l.Parent == null);

            Assert.AreEqual(expected: expectedBottom, actual: foundation.Name);
        }
        ...
        public const string FirstTowerCase = @"
pbga (66)
xhth (57)
ebii (61)
havc (66)
...
";
        ...
        public class TowerLevel
        {
            public TowerLevel()
            {
                ...
                Children = new List<TowerLevel>();
            }

            public string Name { get; set; }
            public int Weight { get; set; }
            public TowerLevel Parent { get; set; }
            public IList<TowerLevel> Children { get; set; }
            public IList<string> ChildrenNames { get; set; }

            public int TotalWeight => WeightRecurse(this);
        ...
        [TestCase(243, "padx")]
        [TestCase(243, "fwft")]
        public void EnsureWeightCalculationWorks(int expectedWeight, string name)
        {
            var towerLevels = CompileTowerLevels(FirstTowerCase);
            var test = towerLevels.Single(l => l.Name.Equals(name));
            Assert.AreEqual(expectedWeight, test.TotalWeight);   
        }
        ...
        /// </summary>
        [TestCase(60, FirstTowerCase)]
        [TestCase(333, TowerPuzzleInput)]
        public void Part2(int expectedNecessaryWeight, string input)
        {
            var towerLevels = CompileTowerLevels(input);
            var foundation = towerLevels.Single(l => l.Parent == null);

            var problemLevel = foundation;

            var subTowerWeightGroups = problemLevel.Children.Select(c => c.TotalWeight).GroupBy(w => w);

            var neededDiff = subTowerWeightGroups.WhereLengthIsAtLeast(2).First().Key - subTowerWeightGroups.WhereLengthIs(1).Single().Key;

            var nextProblemLevel = problemLevel;

            while (nextProblemLevel != null)
            {
                problemLevel = nextProblemLevel;
                nextProblemLevel =
                    problemLevel
            ...
            Assert.AreEqual(expectedNecessaryWeight, weightChangedTo);
        }

        public const string TowerPuzzleInput = @"
apcztdj (61)
