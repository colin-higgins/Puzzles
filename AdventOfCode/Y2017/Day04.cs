...
namespace Puzzles.AdventOfCode.Y2017
{
    ...
    public class Day04
    {
        ...
        /// </summary>
        [TestCase(1, "aa bb cc dd ee")]
        [TestCase(0, "aa bb cc dd aa")]
        [TestCase(1, "aa bb cc dd aaa")]
        [TestCase(3, @"aa bb cc dd aaa
wakka flokka shocka wakka
wakka flokka shocka     
b a d m i t tt on 
ww www ww asd 
")]
        ...
        public void Part1(int expectedValidCount, string input)
        {
            ...
            foreach (var row in rows)
            {
                ...
                var valid = true;
                foreach (var column in row.SplitOnWhitespace())
                {
                    valid = valid && hashSet.Contains(column) == false;
                    hashSet.Add(column);
                }
                if (valid)
                    validCount++;
            ...
            Assert.AreEqual(expected: expectedValidCount, actual: validCount);
        }
        ...
        /// </summary>
        [TestCase(1, "abcde fghij")]
        [TestCase(0, "abcde xyz ecdab")]
        [TestCase(1, "a ab abc abd abf abj")]
        [TestCase(1, "iiii oiii ooii oooi oooo")]
        [TestCase(0, "oiii ioii iioi iiio")]
        ...
        public void Part2(int expectedValidCount, string input)
        {
            ...
            foreach (var row in rows)
            {
                ...
                foreach (var column in row.SplitOnWhitespace())
                {
                    var ordered = string.Concat(column.OrderBy(c => c));
                    valid = valid && hashSet.Contains(ordered) == false;
                    hashSet.Add(ordered);
                }
                if (valid)
                    validCount++;
        ...
        public const string PassphraseList = @"
