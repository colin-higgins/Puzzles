...
namespace Puzzles.AdventOfCode.Y2017
{
    ...
    public class Day08
    {
        public const string ExampleRegisterInstructions = @"
b inc 5 if a > 1
a inc 1 if b < 5
c dec -10 if a >= 1
c inc -20 if c == 10
";
        ...
        public static Dictionary<string, Func<int, int, bool>> Comparers = new Dictionary<string, Func<int, int, bool>>
        {
            { ">", (a,b) => a > b},
            { ">=", (a,b) => a >= b},
            { "<", (a,b) => a < b},
        ...
        public void Part1(int expectedResult, string input)
        {
            ...
            Action<string, int> set = (key, newValue) =>
            {
            ...
            {
                var op = Instruction.Parse(row);

                if (Comparers[op.CompareOperator](get(op.CompareLeft), get(op.CompareRight)))
                    set(op.Variable, Operators[op.ChangeOperator](get(op.Variable), get(op.ChangeInput)));
            }

            var largestValue = registry.Values.Max();

            Assert.AreEqual(expectedResult, largestValue);
        }
        ...
        public class Instruction
        {
            ...
            {
                var parts = input.SplitOnWhitespace().ToArray();
                return new Instruction()
                {
                    Variable = parts[0],
                    ChangeOperator = parts[1],
                    ChangeInput = parts[2],
                    CompareLeft = parts[4],
                    CompareOperator = parts[5],
                    CompareRight = parts[6],
                };
            }

            public string Variable { get; set; }
            public string ChangeOperator { get; set; }
            public string ChangeInput { get; set; }
            public string CompareLeft { get; set; }
            public string CompareOperator { get; set; }
            public string CompareRight { get; set; }
        }
        ...
        /// </summary>
        [TestCase(10, ExampleRegisterInstructions)]
        [TestCase(4644, PuzzleInput)]
        public void Part2(int expectedResult, string input)
        {
            var registry = new Dictionary<string, int>();
            var max = 0;
            ...
            foreach (var row in rows)
            {
                ...
                max = max < get(op.Variable) ? get(op.Variable) : max;
            }

            Assert.AreEqual(expectedResult, max);
        }
