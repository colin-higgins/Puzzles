...
namespace Puzzles.AdventOfCode.Y2017
{
    ...
    public class Day09
    {
        ...
        ///
        /// Sometimes, instead of a group, you will find garbage.Garbage begins with 'less than'and ends with'greater than'.
        /// Between those angle brackets, almost any character can appear, including { and
        /// }. Within garbage, 'less than' has no special meaning.
        ///
        ...
        [TestCase(16, "{{{},{},{{}}}}")]
        [TestCase(1, "{<a>,<a>,<a>,<a>}")]
        [TestCase(9, "{{<ab>},{<ab>},{<ab>},{<ab>}}")]
        [TestCase(9, "{{<!!>},{<!!>},{<!!>},{<!!>}}")]
        [TestCase(3, "{{<a!>},{<a!>},{<a!>},{<ab>}}")]
        [TestCase(7640, PuzzleInput)]
        public void Part1(int expectedScore, string input)
        {
            input = Ignore.Replace(input, "");
            input = Garbage.Replace(input, "");

            var root = (Grouping)null;
            var current = root;
            var garbageDump = "";

            foreach (var character in input.Select(c => c.ToString()))
            {
                if (character == Open)
                {
                    var child = new Grouping()
                    {
                        Parent = current,
                    };
            ...
            Assert.AreEqual(actual: root.NestedScores, expected: expectedScore);
        }

        [TestCase(0, "<>")]
        [TestCase(17, "<random characters>")]
        [TestCase(3, "<<<<>")]
        [TestCase(2, "<{!>}>")]
        [TestCase(0, "<!!>")]
        [TestCase(0, "<!!!>>")]
        [TestCase(10, "<{o\"i!a,<{i<a>")]
        [TestCase(8, "{{<ab>},{<ab>},{<ab>},{<ab>}}")]
        [TestCase(4368, PuzzleInput)]
        public void Part2(int expectedScore, string input)
        {
            input = Ignore.Replace(input, "");

            var garbageMatches = Garbage.Matches(input);
            var filteredInput = Garbage.Replace(input, "");

            var amountOfGarbage = input.Length - filteredInput.Length - (garbageMatches.Count * 2);

            Assert.AreEqual(actual: amountOfGarbage, expected: expectedScore);
        }
        ...
        public class Grouping
        {
            public Grouping()
            {
                Children = new List<Grouping>();
            }

            public Grouping Parent { get; set; }
            public List<Grouping> Children { get; set; }
            public int Score => (Parent?.Score ?? 0) + 1;
            public int NestedScores => Score + Children.Sum(c => c.NestedScores);

        }

        private static readonly Regex Ignore = new Regex("!.");
        private static readonly Regex Garbage = new Regex("<[^>]*>");
