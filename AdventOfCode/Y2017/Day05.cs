...
namespace Puzzles.AdventOfCode.Y2017
{
    ...
    public class Day05
    {
        ...
        /// </summary>
        [TestCase(5, "0 3  0  1  -3")]
        [TestCase(8, "1 0  -1  1  -1")]
        [TestCase(339351, PuzzleInput)]
        public void Part1(int expectedSteps, string input)
        {
            var actualSteps = 0;

            var numbers = input.SplitToIntegers();

            var jumps = numbers.Select(n => new Jump(n)).ToList();

            var currentIndex = 0;

            while (true)
            {
                var nextIndex = jumps[currentIndex].Offset + currentIndex;

                actualSteps++;

                if (nextIndex < 0 || jumps.Count <= nextIndex)
                    break;

                jumps[currentIndex].Leave();
        ...
        public class Jump
        {
            private int _timesLandedOn;
            private readonly int _originalOffset;
            private readonly int? _decrementAt;

            public Jump(int originalOffset)
            {
                _originalOffset = originalOffset;
            }
            public Jump(int originalOffset, int decrementAt)
            {
                _originalOffset = originalOffset;
                _decrementAt = decrementAt;
            }

            public void Leave()
            {
                if (_decrementAt.HasValue && Offset >= _decrementAt)
                    _timesLandedOn--;
                else
                    _timesLandedOn++;
            }

            public int Offset => _originalOffset + _timesLandedOn;
        }
        ...
        [TestCase(8, "1 0  -1  1  -1")]
        [TestCase(24315397, PuzzleInput)]
        public void Part2(int expectedSteps, string input)
        {
            long actualSteps = 0;

            var numbers = input.SplitToIntegers();

            var jumps = numbers.Select(n => new Jump(n, decrementAt: 3)).ToList();
